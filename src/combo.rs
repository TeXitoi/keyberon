...
impl Combo {
    ...
    pub fn tick(
        &mut self,
        mut vec: heapless::Vec<Event, heapless::consts::U8>,
    ) -> heapless::Vec<Event, heapless::consts::U4> {
        let mut vec_remove: heapless::FnvIndexSet<usize, _> =
            heapless::FnvIndexSet::<_, heapless::consts::U8>::new();
        ...
        for e in vec.into_iter() {
            ...
            for (combo_idx, (((x1, y1), (x2, y2), (x3, y3)), ref mut pressed1, ref mut pressed2)) in
                self.stacked.iter_mut().enumerate()
            {
                match e {
                    Event::Release(i, j) => {
